...
class ScaleComponent(ControlSurfaceComponent):
    ...
    def __init__(self, control_surface = None, enabled = False, mode = "diatonic", *a, **k):
        #Live.Base.log("ScaleComponent __init__")
        self._layout_set = False
        #Arma una lista de Pares [NombreEscala,Grado] para los modos
        self._modus_list = [Modus(MUSICAL_MODES[v], MUSICAL_MODES[v + 1]) for v in xrange(0, len(MUSICAL_MODES), 2)]
        #Arma una lista de NombreEscala
        ...
        self._interval = 3 # intervalo entre las filas (nota fila superior = nota fila + _interval) 
        self._matrix = None
        ...
        #self._control_surface.log_message("ScaleComponent.__init__ finish")
        
        
    @property
    def notes(self):
        #self._control_surface.log_message("ScaleComponent.notes")
        return self.modus.scale(self._key).notes

    @property
    def modus(self):
        #self._control_surface.log_message("ScaleComponent.modus")
        return self._modus_list[self._modus]

    def set_key(self, key, message = True):
        #self._control_surface.log_message("ScaleComponent.set_key key=" + str(key) + " message=" + str(message))
        if key>=0 and key<=11:
            self._key = key % 12
            if message:
    ...
    def set_octave(self, octave, message = True):
        #self._control_surface.log_message("ScaleComponent.set_octave octave=" + str(octave) + " message=" + str(message))
    ...
    def octave_up(self, message = True):
        #self._control_surface.log_message("ScaleComponent.octave_up message=" + str(message))
        self.set_octave(self._octave + 1, message) 
    
    def octave_down(self, message = True):
        #self._control_surface.log_message("ScaleComponent.octave_down message=" + str(message))
        self.set_octave(self._octave - 1, message) 
        
    def set_modus(self, index, message = True):
        #self._control_surface.log_message("ScaleComponent.set_modus index=" + str(index) + " message=" + str(message))
    ...
    def set_drumrack(self, drumrack):
        #self._control_surface.log_message("ScaleComponent.set_modus set_drumrack=" + str(drumrack))
    ...
    #        self.update()
    def set_matrix(self, matrix):
        #self._control_surface.log_message("ScaleComponent.set_matrix matrix=" + str(matrix))
        self._matrix = matrix
        if matrix:
            matrix.reset()
    ...
    def set_osd(self, osd):
        #self._control_surface.log_message("ScaleComponent.set_osd osd=" + str(osd))
        self._osd = osd

    def _update_OSD(self):
        #self._control_surface.log_message("ScaleComponent._update_OSD")
        if self._osd != None:
    ...
    def update(self):
        #self._control_surface.log_message("ScaleComponent.update")
    ...
    def _matrix_pressed(self, value, x, y, is_momentary):
        #self._control_surface.log_message("ScaleComponent._matrix_pressed value=" + str(value) + " x=" + str(x) + " y=" + str(y) + " is_momentary=" + str(is_momentary))
    ...
    def matrix_release(self, pad):
        #self._control_surface.log_message("ScaleComponent.matrix_release")
        pass
    ...
    def is_diatonic(self):
        #self._control_surface.log_message("ScaleComponent.is_diatonic")
    ...
    def is_chromatic(self):
        #self._control_surface.log_message("ScaleComponent.is_chromatic")
        return not self.is_drumrack  and  (self._mode =="chromatic" or self._mode == "chromatic_gtr")

    @property
    def is_diatonic_ns(self):
        #self._control_surface.log_message("ScaleComponent.is_diatonic_ns")
        return self._mode == "diatonic_ns"
    
    @property    
    def is_chromatic_gtr(self):
        #self._control_surface.log_message("ScaleComponent.is_chromatic_gtr")
    ...
    def get_pattern(self):
        #self._control_surface.log_message("ScaleComponent.get_pattern")
        notes = self.notes
        # origin
...
class Modus(Scale):
    ...
    def scale(self, base_note):
        #Live.Base.log("Modus.base_note- base_note=" +str(base_note))
        return Scale(KEY_NAMES[base_note], [base_note + x for x in self.notes])

    def scales(self, base_notes):
        #Live.Base.log("Modus.scales - scales=" +str(scales))
...
class MelodicPattern(object):
    ...
    def __init__(self,
        ...
        #Live.Base.log("MelodicPattern.__init__")
        self.steps = steps
        self.scale = scale
    ...
    class NoteInfo:
        ...
        def __init__(self, index, channel, root = False, highlight = False, in_scale = False, valid = False):
            #Live.Base.log("NoteInfo.__init__")
            self.index = index
    ...
    def _extended_scale(self):
        #Live.Base.log("MelodicPattern._extended_scale")
    ...
    def _octave_and_note(self, x, y):
        #Live.Base.log("MelodicPattern._octave_and_note x=" + str(x) + ", y=" + str(y))
        ...
        #Live.Base.log("MelodicPattern._octave_and_note END - Returning octave=" + str(octave) + ", note=" + str(note))
        return (octave, note)

    def note(self, x, y):
        #Live.Base.log("MelodicPattern.note x=" + str(x) + ", y=" + str(y))
        ...
        #Live.Base.log("MelodicPattern.note END - Returning index=" + str(index) + ", x=" + str(x) + ", root=" + str(root) + ", highlight=" + str(highlight) + ", in_scale=" + str(in_scale) + ", valid=" + str(valid))
