		...
		if self.is_enabled():
			...
			if self._device is None and self._matrix is not None:
				for x in range(self._matrix.width()):
					for y in range(self._matrix.height()):
						if self._force:
							self._matrix.get_button(x, y).set_on_off_values("DefaultButton.Disabled","DefaultButton.Disabled")
	...
	def update_track_buttons(self):
		...
		if self.is_enabled():
			...
			if(self._next_track_button is not None):
				...
				if self.selected_track_idx is not None and self.selected_track_idx < len(self.song().tracks) - 1 and not self._is_locked_to_device:
	...
	def update_device_buttons(self):
		...
		if self.is_enabled():
			if(self._prev_device_button is not None):
				...
				if self.selected_track().devices is not None and (len(self.selected_track().devices) > 0 and self.selected_device_idx > 0 and not self._is_locked_to_device):
